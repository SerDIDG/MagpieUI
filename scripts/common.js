...
cm.isRegExp = function(obj){ return obj.constructor == RegExp; };
cm.toFixed = function(n, x){ return	parseFloat(n).toFixed(x); };
cm.toNumber = function(str){ return parseInt(str.replace(/\s+/, '')); }
...
cm.getX = function(o){
	var x=0;try{while(o){x+=o.offsetLeft;o=o.offsetParent;}}catch(e){return x;};return x;
};

cm.getY = function(o){
	var y=0;try{while(o){y+=o.offsetTop;o=o.offsetParent;}}catch(e){return y;};return y;
};
...
cm.easing = function(type, duration){
	var epsilon = (1000 / 60 / duration) / 4,
		types = {
			'easeIn' : function(progress){ return Math.pow(progress, 3); },
			'easeOut' : function(progress){ return 1 - types['easeIn'](1 - progress); },
...
cm.cookieSet = function(name, value, expires){
	document.cookie = escape(name) + "=" + escape(value) + ';' + (expires? cm.cookieDate(expires) : '');
};
...
cm.cookieDate = function(num){
	return 'expires=' + (new Date(new Date().getTime () + 1000 * 60 * 60 * 24 * num)).toGMTString() + ';';
};
