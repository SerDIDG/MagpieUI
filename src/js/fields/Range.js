...
cm.getConstructor('Com.Range', function(classConstructor, className, classProto, classInherit){
    classProto.renderRangeContent = function(){
        ...
        // Structure
        nodes.container = cm.node('div', {classes: 'com__range__field'},
            nodes.bar = cm.node('div', {classes: 'bar'},
                nodes.range = cm.node('div', {classes: 'range'})
            ),
            cm.node('div', {classes: 'min'}, cm.parseMessage(that.params.valueFormater, {value: that.params.min})),
            cm.node('div', {classes: 'max'}, cm.parseMessage(that.params.valueFormater, {value: that.params.max})),
        );
