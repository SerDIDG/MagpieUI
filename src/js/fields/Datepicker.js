...
function(params) {
    ...
    var render = function() {
        ...
        if (that.params.showClearButton) {
            ...
            nodes.container.appendChild(
                nodes.clearButton = cm.node('div', {
                    class: that.params.icons.clear,
                    title: that.lang('Clear date')
                })
            );
        }
        // Today / Now Button
    ...
    var setLogic = function() {
        ...
        // Clear Button
        if (that.params.showClearButton) {
            cm.click.add(nodes.clearButton, () => {
                that.clear();
        ...
        // Today / Now Button
        if (that.params.showTodayButton) {
            cm.click.add(nodes.todayButton, () => {
                that.set(new Date());
                components.menu.hide(false);
            });
        }
    ...
    var renderCalendar = function() {
        // Render Calendar
        cm.getConstructor(that.params.calendar.constructor, classConstructor => {
            components.calendar = new classConstructor(
                cm.merge(that.params.calendar.constructorParams, {
        ...
        // Render Time Select
        if (that.params.isDateTime) {
            cm.getConstructor(that.params.timeSelect.constructor, classConstructor => {
                components.time = new classConstructor(
                    cm.merge(that.params.timeSelect.constructorParams, {
                        container: nodes.timeContainer,
                        minutesInterval: that.params.minutesInterval,
                    })
                );

                // Add a change event only after a component has been rendered
                components.time.addEvent('onChange', () => {
                    setDate();
